...
<body>
	...
	<%
		...
		List<TripDetailsRemoved> removedEmployee=(List<TripDetailsRemoved>)request.getAttribute("removedEmployee");
	...
	<div id="body">
		<div class="content">
			<form action="updateTripSheetSubmit" name="tripmodifyform" id="tripmodifyform"
				method="POST">
				...
				<table border="0" >
					<tbody>
						...
						<%		if(removedEmployee!=null&&removedEmployee.size()>0)
									{										
										for (TripDetailsRemoved tdr : removedEmployee) {									
										%>
										<option
										value="<%=tdr.getEmployee().getId() + ":"
							+ tdr.getLandmark().getId() +":"+tdr.getEmployeeSchedule().getId()%>">
										<%=tdr.getEmployee().getEmployeeFirstName() + "--"
							%></option>
								<%}
								}
						 %>
							...
							<td>	
								...
								name="trip<%=i%>" multiple="" size="3" style="width: 300px">
									...
									<%
									TripDetailsService tds=(TripDetailsService)request.getAttribute("tripDetailsService");
									if(i<tripIds.length)
									{	
									String tripId = tripIds[i];
									td = tds.getTripsById(tripId);
							...
							%>																					
									...
									%>
									<option
										value="<%=tdc.getEmployee().getId() + ":"
							+ tdc.getLandmark().getId() +":"+tdc.getEmployeeSchedule().getId()+":"+tdc.getEmployee().getGender()%>">
										<%=tdc.getEmployee().getEmployeeFirstName()+" "+tdc.getEmployee().getEmployeeFirstName() + "--"
							+tdc.getEmployee().getAddress()%></option>
									<%
										}
								...
								<td>Escort
								<select name="escort<%=i%>" id="escort<%=i%>"><option value="YES">YES</option></select></td>
								<td>Vehicle Type<select name="vehicleType<%=i%>" id="vehicleType<%=i%>">
								<%for(VehicleType vt: vehicleTypes) {%>
								<option value="<%=vt.getId()+":"+vt.getSitCap()%>"><%=vt.getType() %></option>
								<%} %>
								</select></td>	
