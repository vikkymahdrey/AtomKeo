...
@Repository
public class OtherDaoImpl extends AbstractDao implements OtherDao {
	private static final Logger logger = Logger.getLogger(OtherDaoImpl.class);
	...
	public String[] getCity(String site) throws Exception {
		...
		try {
			...
			for(Object[] o : oList) {
				city[0]=String.valueOf(o[0]);
				city[1]=String.valueOf(o[1]);
			}
		} catch (Exception e) {
			logger.error("Exception in getCity" + e);
		}
	...
	public String getEmployeeDet(String empId) {
		...
		String query="select employee.id,loginId,displayname as  EmployeeName , EmailAddress,PersonnelNo,LineManager,StaffManager,DeptNo,OperationCode,case when employee.registerStatus is null or employee.registerStatus!='a' then 'Not Registered'  else 'Registered'   end registerStatus,  case when gender='m' then 'Male' else 'Female' end gender,case when employee_subscription.subscriptionStatus is null then 'Not Subscribed' when employee_subscription.subscriptionStatus ='subscribed' then 'Subscribed' when employee_subscription.subscriptionStatus ='pending' then 'Pending'  end subscriptionStatus,address,  contactNumber1  contactNumber,addressChangeDate,isContractEmployee as emptype" +
					"" +
			...
			for(Object[] o:oList){
				...
				returnString += " <tr> " + "<td align='right' > "
						...
						+ String.valueOf(o[2]) + "</td> "
						+ " <td  align='right'> <b> Status</b>"
						+ " </td> " + "<td  > " + "<b>: </b>" + "</td> " +

						" <td  align='left'> "
						+ String.valueOf(o[9]) + " </td> "
						+ "</tr>";
				returnString += " <tr> "
						+ "<td  align='right'> <b> My ID </b> </td> "
						+ "<td  > " + "<b>: </b>" + "</td> "
						+ " <td  width='20%' align='left'> "
						+ String.valueOf(o[4]) + " </td> "
						+ " <td  align='right'> <b> Gender</b>" + " </td> "
						+ "<td  > " + "<b>: </b>" + "</td> " +

						" <td  align='left'> " + String.valueOf(o[10])
						+ " </td> " +

						"</tr>";

				employeeAddress = "" + String.valueOf(o[12]);
				empType =String.valueOf(o[15]);
				String afterthirtydayDate = OtherFunctions.getafterthirtydayDateInSqlFormat();
				if ( ((String.valueOf(o[17]) == null) || ((String.valueOf(o[17])).compareTo(afterthirtydayDate) > 0))) {
			...
			String query1="select date_format(from_date,'%d-%m-%Y') from_date,date_format(to_date,'%d-%m-%Y') to_date from employee_schedule where employee_id="
							+ empId
			...
			for(Object[] obj: objList){
				returnString += " <tr> "
						...
						" <td  align='left'> " + String.valueOf(obj[1])
						+ " </td> " + "</tr>";
