...
<body>
	<div id="body">
		<div class="content">
			...
			ArrayList<TripDetailsDto> vendorAssignTrip = (ArrayList<TripDetailsDto>)request.getAttribute("vendorAssignTrip");
			List<Vendormaster> masterVendorList = (List<Vendormaster>)request.getAttribute("masterVendorList");
			...
			<form action="assignTripsToVendor" name="assignvendorform" id="assignvendorform"
				method="POST">
				...
				type="hidden" name="tripTime" id="tripTime" value="<%=tripTime%>" />
						...
						<tr>
							...
							<td align="left"><select id="vendor" name="vendor">
									<%
										for (Vendormaster vendor : masterVendorList) {
									%>

									<option value="<%=vendor.getId()%>"><%=vendor.getCompany()%></option>
									<%
										}
									%>
							</select></td>
