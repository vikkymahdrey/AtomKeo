...
public class ReportController {
	...
	public String modifytrip(HttpServletRequest request) throws Exception{
		...
		EmployeeScheduleAlter esa=tripDetailsService.ModifySchedule(schid, time, logtype,date);
		if(esa!=null){
			try{
			if(esa.getLogoutTime().equalsIgnoreCase("Cancel")){
			String subject = "Booking Cancellation";	
			String message = settingsService.getEmployeeBookingCancellationMsg(esa,oldtime);
			SendMail mail = SendMailFactory.getMailInstance();
			mail.send(esa.getEmployeeSchedule().getEmployee1().getEmailAddress(), subject, message);
			} 
			else if(!esa.getLogoutTime().equalsIgnoreCase(oldtime)){
				String subject = "Booking Modification";	
				String message = settingsService.getEmployeeBookingModificationMsg(esa);
				SendMail mail = SendMailFactory.getMailInstance();
				mail.send(esa.getEmployeeSchedule().getEmployee1().getEmailAddress(), subject, message);
			} 
			  }catch(Exception e){
				  logger.debug("error-booking for cancellation or modification",e);
			  }
			return "true";
		 }else{
		  return "false";
	...
	public ModelAndView modifytripview(HttpServletRequest request) throws Exception{
		...
		long timediff=es.getToDate().getTime()-es.getFromDate().getTime();
		...
		for(int i=0;i<=daysdiff;i++){
			...
			//if(ess.getFromDate().equals(new Date())||ess.getFromDate().after(new Date()))
			list.add(ess);
		}
