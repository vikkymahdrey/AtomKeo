...
<%@page import="com.agiledge.atom.config.files.PaginationService"%> 
...
<%@page import="com.agiledge.atom.entities.EmployeeSubscription"%>
<%@ page contentType="text/html; charset=utf-8" language="java"
	 errorPage="errorInJSP.jsp"%>
...
<script type="text/javascript">
	...
	function searchForm() {
		...
		document.siteSearchForm.action = "schedulingOpenByTransadmin";
		// document.siteSearchForm.method = "POST";
 ...
 function copyTimesSpecificToProject(element) {
	...
	function showAll() {
		...
		var startPos = 0;
		var endPos = listSize;
		window.location = "schedulingOpenByTransadmin?site=" + site + "&startPos="
	...
	function next() {
		...
		if (startPos > listSize) {
			...
			window.location = "schedulingOpenByTransadmin?site="+ site
...
<body>
	...
	<div id="body">
		...
		List<Logtime> masterLoginTimeList=(List<Logtime>)request.getAttribute("masterLoginTimeList");
		List<Logtime> masterLogoutTimeList=(List<Logtime>)request.getAttribute("masterLogoutTimeList");
		List<Employee> empBySiteAndSubscription=(List<Employee>)request.getAttribute("empBySiteAndSubscription");
		List<Employee> empBySiteAndSubscriptionIterated = new PaginationService().getNext(empBySiteAndSubscription, startPos, endPos);
	  if (RECORD_SIZE > empBySiteAndSubscription.size()) {
		endPos = "" + empBySiteAndSubscription.size();
	}
	int serialNo = 0;
				if (empBySiteAndSubscription != null && empBySiteAndSubscription.size() > 0) {
			%>
			...
			<tr>
					...
					<td id="pageInfo"><%=Integer.parseInt(startPos) + 1%> to <%=endPos%>
						of</td>
					<td><%=empBySiteAndSubscription.size()%> is showing</td>
			...
			<form name="schedule_emp" method="post" action="scheduleEmployee"
				...
				<table class="dataTable">
					...
					<%
					List<Logtime> loginTimeList=null;
					List<Logtime> logoutTimeList=null;
					
						for (Employee empSubs : empBySiteAndSubscriptionIterated) {
								serialNo++;
								loginTimeList=new ArrayList<Logtime> ();
								logoutTimeList=new ArrayList<Logtime> ();		
					%>
					<tr id="<%=serialNo%>">
						<td><input class="checkEmployee" size="1" type="checkbox"
							id="employeecheck<%=serialNo%>" name="subscriptionId"
							value="<%=empSubs.getEmployeeSubscriptions1().get(0).getSubscriptionID()%>" /></td>
						<td><%=empSubs.getLoginId()%></td>

						<td><%=empSubs.getDisplayname()%></td>

						<td class="td-EffectiveDate"><%=empSubs.getEmployeeSubscriptions1().get(0).getFromDate()%> 
						<input type="hidden"
							id="subscriptionEffectiveDate-<%=serialNo%>"
							value="<%=empSubs.getEmployeeSubscriptions1().get(0).getSubscriptionDate()%>" /></td>


						<td><%=empSubs.getAtsconnect().getProject()%>
						<input type="hidden" class="project"
							name="project<%=empSubs.getEmployeeSubscriptions1().get(0).getSubscriptionID()%>"
							id="project<%=serialNo%>"
							value="<%=empSubs.getAtsconnect().getId()%>" /></td>
						<td class="td-schedulingFromDate"><input readonly="readonly"
							name="from_date<%=empSubs.getEmployeeSubscriptions1().get(0).getSubscriptionID()%>"
						...
						<td><input class="toDate" readonly="readonly"
							name="to_date<%=empSubs.getEmployeeSubscriptions1().get(0).getSubscriptionID()%>"
						...
						<td><input type="checkbox"
							name="weeklyoff<%=empSubs.getEmployeeSubscriptions1().get(0).getSubscriptionID()%>"
							class="weeklyoff" /></td>
							
							<%
								logtimeList=empSubs.getAtsconnect().getLogtimes();
							     				      
							      for(Logtime logtime: logtimeList){
									if(logtime.getStatus().equals("active")){
									 if(logtime.getLogtype().equals("IN")){
										 loginTimeList.add(logtime); 
									 }else{
										logoutTimeList.add(logtime);
									        } 
									   }
								 } 
								 loginTimeList.addAll(masterLoginTimeList);
								 logoutTimeList.addAll(masterLogoutTimeList);
								 if (empSubs.getAtsconnect().getId()== null
											&& empSubs.getAtsconnect().getId().equals("")) {
										loginTimeList = masterLoginTimeList;
										logoutTimeList = masterLogoutTimeList;
									} %>
							
						<td id="td-login<%=serialNo%>"><select class="loginTime"
							name="logintime<%=empSubs.getEmployeeSubscriptions1().get(0).getSubscriptionID()%>"
							id="logintime<%=serialNo%>">

								<option value="">Select</option>
								<option value="none">None</option>
								<%
								 for (Logtime logIN : loginTimeList) {
								%>
								<option value="<%=logIN.getLogtime()%>"><%=logIN.getLogtime()%></option>
								<%
									}
								%>
						...
						<td id="td-logout<%=serialNo%>"><select class="logoutTime"
							name="logouttime<%=empSubs.getEmployeeSubscriptions1().get(0).getSubscriptionID()%>"
							id="logouttime<%=serialNo%>">

								<option value="">Select</option>
								<option value="none">None</option>
								<%
									for (Logtime logOUT : logoutTimeList) {
								%>
								<option value="<%=logOUT.getLogtime()%>"><%=logOUT.getLogtime()%></option>
								<%
									}
								%>
						</select> <img src="images/error.png" class="errorImage"
						  ...
						  <td><input type="hidden" class="scheduledTo"
							...
							value="<%=empSubs.getEmployeeSchedules1().get(0).getToDate()%>" /> <input
							type="hidden" class="scheduledFrom"
							id="scheduledFrom<%=serialNo%>"
							value="<%=empSubs.getEmployeeSchedules1().get(0).getFromDate()%>" /> <%
 	//System.out.println("scheduled to"+schdulingEmployeeDto.getSchedulingToDate());

							 
		                        EmployeeSchedule latestSchedule = empSubs.getEmployeeSchedules1().get(0);
		                        for(EmployeeSchedule eachSchedule : empSubs.getEmployeeSchedules1()){
		                        	if(eachSchedule.getToDate() != null && eachSchedule.getToDate().compareTo(latestSchedule.getToDate()) > 0)
		                        		latestSchedule = eachSchedule;
		                        } 
		                       
		                        
 			if (!(("" + latestSchedule.getToDate())
 					.equals("null"))) {
 				out.print("Active</td><td>"
 						+ latestSchedule.getToDate());
 			}
 ...
 %></td>
						...
						<td><%=latestSchedule.getEmployee2().getId()%></td>
					<%-- 	<td><%=OtherFunctions.changeDateFromatToddmmyyyy(empSubs.getEmployeeSchedules1().get(0).getStatusDate())%></td> --%>
						<td><%=latestSchedule.getStatusDate()%></td> 
			...
			<table style="width: 40%">
				...
				<tr>
					<td colspan="4"><input type="hidden" value="<%=RECORD_SIZE%>"
						...
						value="<%=empBySiteAndSubscription.size()%>" id="listSize" /></td>
				</tr>
			</table>
