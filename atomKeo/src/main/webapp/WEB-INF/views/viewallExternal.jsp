...
<tr>
		...
		{
			empList = (List<Employee>)request.getAttribute("empEnabledList");
		} else 
		{
			empList =(List<Employee>)request.getAttribute("empAllList");
		} 
		...
		%>
		<tr>
		<td align="center"><%=e.getPersonnelNo()%></td>
		<td align="center"><%=e.getEmployeeFirstName() %></td>
		<td align="center"><%=e.getEmployeeLastName() %></td>
		<td align="center"><%=e.getGender() %></td>
		<td align="center"><%=e.getContactNumber1() %></td>
		<td align="center"><%=e.getEmailAddress() %>
		<input type="hidden" name="address<%=e.getPersonnelNo()%>" value="<%=e.getAddress() %>"/>
		<input type="hidden" name="linename<%=e.getPersonnelNo()%>" value="<%=e.getEmployeeSubscriptions2().get(0).getEmployee2().getDisplayname()%>"/>
		<input type="hidden" name="fisname<%=e.getPersonnelNo()%>" value="<%=e.getEmployeeFirstName()%>"/>
		<input type="hidden" name="midname<%=e.getPersonnelNo()%>" value="<%=e.getEmployeeMiddleName()%>"/>
		<input type="hidden" name="lasname<%=e.getPersonnelNo()%>" value="<%=e.getEmployeeLastName()%>"/>
		<input type="hidden" name="dsname<%=e.getPersonnelNo()%>" value="<%=e.getDisplayname()%>"/></td>
		
		
		<td align="center"><%=status%>&nbsp;&nbsp;&nbsp;&nbsp;<input name="statusbutton" type="button" class="formbutton" value="<%=value %>" onClick="updateStatus(<%=e.getId() %>,<%=e.getActive() %>)" /></td>
		<td align="left">&nbsp;&nbsp;&nbsp;&nbsp;<img src="images/edit1.png" class="editButton" title="Edit" onClick='showEditArea("<%=e.getPersonnelNo()%>","<%=e.getContactNumber1() %>","<%=e.getEmailAddress() %>","<%=e.getGender()%>","<%=e.getAuthType() %>","<%=e.getUsertype()%>","<%=e.getAtsconnect().getProject()%>","<%=e.getEmployeeSubscriptions2().get(0).getEmployee2().getId()%>","<%=e.getAtsconnect().getId() %>")'/>
		&nbsp;&nbsp;&nbsp;&nbsp;<img src="images/project.png" title="Assign Project" onClick='showproject("<%=e.getId() %>","<%=e.getPersonnelNo() %>","<%=e.getDisplayname() %>","<%=e.getAtsconnect().getProject() %>","<%=e.getAtsconnect().getId() %>")'/>
		<%
		
		if(e.getAuthType().equalsIgnoreCase("l"))
		{
		%>
		&nbsp;&nbsp;&nbsp;&nbsp;<input type="button"  id="pwdresetbutton" class="formbutton" name="pwdresetbutton" value="Password Reset" onClick="resetPassword(<%=e.getId()%>)"/>
		<%
		}
		%>
		</td>
		...
		<tr>
		<td align="left">&nbsp;&nbsp;&nbsp;&nbsp;User Type</td><td align="left"><select name="usertype" id="usertype"> <option value="emp">Process Associate</option><option value="PM">Process Manager</option><option value="v">Vendor</option><option value="ta">Transport Admin</option><option value="tm">Transport Manager</option><option value="hrm">Team lead</option><option value="spoc">SPOC</option></select>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
		<tr>
