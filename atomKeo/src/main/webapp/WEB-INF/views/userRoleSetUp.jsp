...
<%@page import="com.agiledge.atom.entities.Page"%>
...
<script type="text/javascript">
 	...
 	*/
 	function showRoleView(roleName,roleId ) {
 		...
 		self.location = "roleView?roleId=" + encodeURIComponent(roleId) + "&roleName=" + encodeURIComponent(roleName);
...
//		var address = $("input[id=address]").val();
	...
	function fillFields(id) {
		...
		$("form[name='UserRoleSetUpForm']").attr("action","updateRole");
...
<body>
	...
	List<Role> userTypes = (List<Role>) request.getAttribute("userTypes");
			%>
	...
	<div id="body">
		<div class="content">
			...
			<hr />
			<form name="UserRoleSetUpForm" action="addNewRole" method="post"
			...
			<disp:dipxTable  id="row" list="<%=userTypes %>" style="width:100%;" >
				...
				%>
					<select   name="pageDrp" id="pageDrp"   >
						...
						<% if(pages!=null&&pages.size()>0) {
							...
							%>
								<option value="<%=p.getId()%>"><%=p.getUrl()%></option>
						<%
								}
						}%> 
					</select>
