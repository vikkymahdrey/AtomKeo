...
//		}
	...
	public List<EmployeeSchedule> scheduleList(String imei) {
		...
		Employee emp= getEmployeeDetails(imei);
		Query q = getEntityManager().createQuery(
				"FROM EmployeeSchedule where toDate>= :todate");
		

		q.setParameter("todate", new Timestamp(cal1.getTimeInMillis()));
		List<EmployeeSchedule> schedule= (List<EmployeeSchedule>)q.getResultList();
		return schedule;
	}

	public String requestScheduleAlter(int scheduleid, String date,
			String login, String logout) {
		// Checking only LogOut
		Date date1=OtherFunctions.stringToDate(date);
		Date date2=new Date();
		String status="Failed";
		boolean flag=true;
		EmployeeScheduleAlter ret=null;
		if(date1.getYear()==date2.getYear() && date1.getMonth()==date2.getMonth() && date1.getDate()==date2.getDate()){
			String hour=logout.split(":")[0];
			String min=logout.split(":")[1];
			if(Integer.parseInt(hour)>date2.getHours()){
				int diff=Integer.parseInt(hour)-date2.getHours();
				int secdiff=Integer.parseInt(min)+(60*diff)-date2.getMinutes();
				if(secdiff<60){
					flag=false;
					status="Booking alter is restricted within 1 hour of schedule departure";
				}
			}
		}
		if(flag==true){
			EmployeeSchedule schedule=getEntityManager().find(EmployeeSchedule.class, scheduleid);
			EmployeeScheduleAlter schedulealter=getScheduleAlterOnDate(scheduleid,date);
			Employee emp = schedule.getEmployee1();
			if(schedulealter!=null){
				schedulealter.setLoginTime(login);
				schedulealter.setLogoutTime(logout);
				schedulealter.setEmployee(emp);
				ret=(EmployeeScheduleAlter)update(schedulealter);
			}else{
				EmployeeScheduleAlter newesa = new EmployeeScheduleAlter();
				newesa.setEmployee(emp);
				newesa.setDate(date1);
				newesa.setEmployeeSchedule(schedule);
				newesa.setLoginTime(login);
				newesa.setLogoutTime(logout);
				ret=(EmployeeScheduleAlter)update(newesa);
			}
		}
		if(ret!=null){
			status="Booking altered successfully";
		}
		return status;
	}

	private EmployeeScheduleAlter getScheduleAlterOnDate(int scheduleid,
			String date) {
		
		Query q = getEntityManager().createQuery(
				"FROM EmployeeScheduleAlter WHERE employeeSchedule.id=:scheduleid and date:=date");

		q.setParameter("scheduleid", scheduleid);
		q.setParameter("date", date);
		EmployeeScheduleAlter esa=null;
		if(q.getResultList()!=null){
			esa = (EmployeeScheduleAlter) q.getResultList().get(0);
		}
		return esa;
	}
	...
	public String transportrequest(String imei, String fromDate, String toDate,
		...
		String status="Transport Booking failed";
		Employee emp=getEmployeeDetails(imei);
		Date date1=OtherFunctions.stringToDate(fromDate);
		Date date2=OtherFunctions.stringToDate(toDate);
		Date date3=new Date();
		boolean flag=true;
		if(date1.getYear()==date3.getYear() && date1.getMonth()==date3.getMonth() && date1.getDate()==date3.getDate()){
			String hour=logOut.split(":")[0];
			String min=logOut.split(":")[1];
			if(Integer.parseInt(hour)>date3.getHours()){
				int diff=Integer.parseInt(hour)-date3.getHours();
				int secdiff=Integer.parseInt(min)+(60*diff)-date3.getMinutes();
				if(secdiff<60){
					flag=false;
					status="Booking is restricted within 1 hour of schedule departure";
				}
			}
		}
		if(flag){
		Query q = getEntityManager().createQuery("FROM EmployeeSchedule where employee1.id= :empid and toDate  >=  :fromdate");
		q.setParameter("empid", emp.getId());
		q.setParameter("fromdate", fromDate);
		EmployeeSchedule schedule=null;
		if(q.getResultList().size()>0){
			schedule=(EmployeeSchedule)q.getResultList().get(0);
			status="Schedule Already Exist For These Dates";
		}else{
			schedule = new EmployeeSchedule();
			schedule.setEmployee1(emp);
			schedule.setEmployee2(emp);
			schedule.setLoginTime(logIn);
			schedule.setLogoutTime(logOut);
			schedule.setFromDate(date1);
			schedule.setToDate(date2);
			schedule.setStatus("a");
			EmployeeSchedule retobj=(EmployeeSchedule)update(schedule);
			if(retobj!=null){
				status="Transport Booking successful";
			}
		}
		}
		return status;
